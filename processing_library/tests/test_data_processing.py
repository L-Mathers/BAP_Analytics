...
import pandas as pd
import pytest
from battery_analysis.data_processing import add_cv_steps, psuedo_limit_extraction
...
def test_add_cv_steps():
    ...
    assert (
        "charge cv" in df_processed["Step Type"].unique()
        or "discharge cv" in df_processed["Step Type"].unique()
    )
