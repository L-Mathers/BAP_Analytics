...
###############################################################################
# CORE LOGIC FOR GROUPING / PROCESSING DATA
###############################################################################
...
def psuedo_limit_extraction(df: pd.DataFrame, zero_current_tolerance: float = 0.1):
    ...
    print(avg_last_volt_charge, avg_last_volt_discharge)    
    return (
...
def _build_per_cycle_df(groups, targets, key_mapping, c_rate_tol):
    """
    Builds a DataFrame with one row per cycle.
    Each target key is a column.
    For each (cycle, target) match, we fill that cycle's row in the corresponding column.
    """

    # Gather all per-cycle target keys
    all_keys = [t["key"] for t in targets]
    if not all_keys:
        return pd.DataFrame()
    ...
    # Initialize DataFrame with NaN (one row per cycle, columns = target keys)
    df_pc = pd.DataFrame(np.nan, index=range(len(cycles)), columns=all_keys)
    df_pc.insert(0, "cycle", cycles)  # Keep cycle as the first column for reference

    # Map cycle values to DataFrame row indices for quick updates
    cycle_to_index = {cyc: i for i, cyc in enumerate(cycles)}

    # Fill in values per target
    for tgt in targets:
        tkey = tgt["key"]
        ivar = tgt.get("interest_variable")
        ...
        for grp in groups:
            ...
            if match and ivar in grp:
                ...
                # Determine where to place this value
                row_idx = cycle_to_index[cyc]
                
                # If there's already a value in this column, apply aggregation
                if pd.notna(df_pc.at[row_idx, tkey]):
                    ...
                    if agg == 'min':
                        ...
                        val = (existing_val + val) / 2  # Simple averaging (better to store counts in reality)
                
                df_pc.at[row_idx, tkey] = val  # Assign value in the correct place
...
###############################################################################
# ENTRY POINT: process_lifetime_test
###############################################################################
...
def process_lifetime_test(data: pd.DataFrame, combined_input: dict, config: dict):
    ...
    capacity = combined_input.get("cell_limits", {}).get("capacity")
    user_input = combined_input.get("user_input", None)

    # Merge static + dynamic config
    ...
    result_df.to_csv('inclusive_kpi.csv')
...
# Example user input
rpt = "/Users/liammathers/Desktop/Github/BAP_Analytics/Testing/20240726_150811_Z61_EVE_C1_ZPg_D00_SN14524.csv"

rpt = '/Users/liammathers/Desktop/Github/Testing_Center_Analytics/Electra/Data Monitoring and Processing/electra_s3_local/ForGraphing/EV_SKE_556_020/20250113_SKE_556_020_RPT7_27_T25_Wb_1.csv'
rpt = '/Users/liammathers/Desktop/Github/BAP_Analytics/Testing/EV_SKE_556_019_RPT3_423_T25_15-07-2024_AllData.csv'

combined_dict = {
    "test_type": "Rate Performance Test",
    "cell_limits": {"capacity": 57.5},
    "user_input": {
        # any dynamic stuff user wants
        "pulse_durations": [1, 2],
        "special_crates": [1, 2],
...
with open("Combined_Creation/Static_Dict.json", "r") as f:
...
# cycling = "/Users/liammathers/Desktop/Github/BAP_Analytics/Testing/BMW_LTF_2580_002_10SOC_Cycling_366_T35_26-06-2024_AllData.csv"
cycling = "/Users/liammathers/Desktop/Github/BAP_Analytics/Testing/1738763214_52_1_2C_Cycling_v2_AllData.csv"
cal = '/Users/liammathers/Desktop/Github/BAP_Analytics/Testing/BMW_LTF_2619_001_Cal1_424_T55_25-06-2024_AllData.csv'
df = pd.read_csv(cycling)

results_df = process_lifetime_test(
    data=df,
    combined_input=combined_dict,
    config=base_config,
)